 ...
 return <div className="panel">
    <div className="toolbar">
        <div className="search">
            <Search size={17}/>
            <input placeholder="Search ID, description, counterparty…" value={search} onChange={e=>setSearch(e.target.value)}/>
            </div>
            <select value={month} onChange={e=>setMonth(e.target.value)}>
                <option value="all">All months</option><option>2026-01</option>
                <option>2026-02</option>
                <option>2026-03</option>
                </select>
                </div>
                <Table data={data} setSelected={setSelected}/>
                </div>
}
        ...
        <tbody>
            {data.map(r => (
                <tr key={r.id} onClick={() => setSelected(r)}>
                    ...
                    <td>
                        <label className={r.category.includes("Revenue") ? "pill green" : r.category.includes("COGS") ? "pill amber" : r.category.startsWith("Operating") ? "pill blue" : "pill gray"}>
                            {r.category}
                        </label>
                    </td>
                    <td className={r.amount >= 0 ? "positive" : "negative"}>
                        {money(r.amount)}
                    </td>
                    <td>›</td>
                </tr>
...
function PNL({data,setSelected,tx}){
 return <div className="panel">
    <div className="panelhead">
        <div>
            <h2>Monthly P&L</h2>
            <p>Revenue → COGS → gross profit → payroll → operating expenses → operating profit</p>
        </div>
    </div>
    <table className="pnl">
        <thead>
            <tr>
                <th>Metric</th>
                {data.map(x => <th key={x.month}>{x.month}</th>)}
            </tr>
        </thead>
        <tbody>
            {[["Revenue","revenue"],["COGS","cogs"],["Gross Profit","grossProfit"],["Payroll","payroll"],["Operating Expenses","operatingExpenses"],["Operating Profit","operatingProfit"]].map(([n,k]) => (
                <tr className={k==="operatingProfit"?"total":""} key={k}>
                    <td>{n}</td>
                    {data.map(x => (
                        <td key={x.month} className={x[k]>=0?"positive":"negative"}>
                            {money(x[k])}
                        </td>
                    ))}
                </tr>
    ...
    <div className="evidence">
        ...
        <p>Click a transaction in the Transactions or Review Queue views to inspect the source row used by the calculation. The financial engine uses the categorized transaction ledger, not generated text.</p>
    </div>
</div>
}
function Reviews({data,setSelected}){
 return <div className="panel">
    <div className="reviewintro">
        <div>
            <h2>{data.length} flagged transactions</h2>
            <p>Review items are intentionally visible instead of being silently included in operating P&L.</p>
            </div>
            </div>
            <div className="reviewgrid">{data.map(r=>
                <article key={r.id} onClick={()=>setSelected(r)}>
                    <div className="rhead">
                        <b>{r.id}</b>
                        <span>{r.date}</span>
                        </div>
                        <h3>{r.description}</h3>
                        <p>{r.counterparty}</p>
                        <div>{r.reviewReasons.map(x=>
                            <label className="reason" key={x}>{x}</label>)}
                            </div><strong>{money(r.amount)}</strong>
                            </article>)}</div></div>
}
...
function Drawer({row,close}){
 return <div className="overlay" onClick={close}>
    <div className="drawer" onClick={e=>e.stopPropagation()}>
        ...
        <h2>{row.id}</h2>
        <p className="drawerdesc">
            {row.description}</p>
            <dl>{[["Date",row.date],["Counterparty",row.counterparty],["Method",row.method],["Category",row.category],["Amount",money(row.amount)]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><div className="trace"><b>Calculation trace</b><p>This source row is categorized by the deterministic rule engine. P&L totals are recomputed from the ledger at request time.</p></div></div></div>
}
